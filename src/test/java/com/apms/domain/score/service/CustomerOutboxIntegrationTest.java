...
import com.apms.domain.score.outbox.processor.CustomerEvaluationApprovedEventProcessor;
...
@ContextConfiguration(classes = CustomerOutboxIntegrationTest.MinimalConfig.class)
public class CustomerOutboxIntegrationTest {
    ...
    public static class MinimalConfig {
        ...
        public CanonicalScoreSnapshotService canonicalScoreSnapshotService(
            ...
            org.mockito.Mockito.when(ownerOrganizationService.getOwnerCompanyId()).thenReturn("mock-owner-id");
        ...
        @Bean
        public CustomerEvaluationApprovedEventProcessor approvedEventProcessor(
            ...
            return new CustomerEvaluationApprovedEventProcessor(
                    taskRepository,
    ...
    @Autowired private CustomerEvaluationApprovedEventProcessor approvedEventProcessor;
    ...
    @BeforeEach
    void setUp() {
        JdbcTestUtils.deleteFromTables(jdbcTemplate, "audit_logs", "processed_outbox_events", "score_snapshots", "project_task_submissions", "project_tasks", "projects", "accounts", "role_criterion_rules", "role_score_rule_sets");
        ...
        ruleSet.setEvaluatedRole(CompanyRole.CUSTOMER);
        ...
        ruleSet.getRules().add(createRule(ruleSet, "revenueProfitabilityScore", "0.22", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "purchaseBehaviorScore", "0.15", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "customerLifetimeValueScore", "0.24", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "retentionLoyaltyScore", "0.18", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "growthPotentialScore", "0.11", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "paymentChurnRiskScore", "0.10", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
    ...
    @Test
    void testCustomerApprovalE2E() {
        // 1. Simulate Mongo Approval Transaction creating Version & Event
        Map<String, CriterionSnapshot> criteria = new HashMap<>();
        criteria.put("revenueProfitabilityScore", CriterionSnapshot.builder().rawScore(new BigDecimal("80.0")).build());
        criteria.put("purchaseBehaviorScore", CriterionSnapshot.builder().rawScore(new BigDecimal("70.0")).build());
        criteria.put("customerLifetimeValueScore", CriterionSnapshot.builder().rawScore(new BigDecimal("60.0")).build());
        criteria.put("retentionLoyaltyScore", CriterionSnapshot.builder().rawScore(new BigDecimal("50.0")).build());
        criteria.put("growthPotentialScore", CriterionSnapshot.builder().rawScore(new BigDecimal("90.0")).build());
        criteria.put("paymentChurnRiskScore", CriterionSnapshot.builder().rawScore(new BigDecimal("75.0")).build());
        // overallScore = 80*0.22 + 70*0.15 + 60*0.24 + 50*0.18 + 90*0.11 + 75*0.10 = 68.90

        RoleEvaluationVersion version = new RoleEvaluationVersion();
        version.setId(versionId);
        version.setEvaluationId(evaluationId);
        version.setEvaluatedRole(CompanyRole.CUSTOMER);
        version.setTargetCompanyProfileId("mock-profile-id");
        version.setReferenceCompanyProfileId("mock-owner-id");
        ...
        RoleEvaluationOutboxEvent event = RoleEvaluationOutboxEvent.builder()
                ...
                .projectId(projectId)
                .taskId(taskId)
                .eventType(RoleEvaluationOutboxEventType.CUSTOMER_EVALUATION_APPROVED)
        ...
        // 5. Verify ScoreSnapshot & score of 68.90
        Iterable<ScoreSnapshot> snapshots = scoreSnapshotRepository.findAll();
        assertThat(snapshots).hasSize(1);
        ScoreSnapshot snapshot = snapshots.iterator().next();
        assertThat(snapshot.getOverallScore()).isEqualByComparingTo(new BigDecimal("68.90"));
        assertThat(snapshot.getEvaluatedRole()).isEqualTo(CompanyRole.CUSTOMER);
