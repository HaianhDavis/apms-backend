...
import com.apms.domain.score.outbox.processor.SupplierEvaluationApprovedEventProcessor;
...
@ContextConfiguration(classes = SupplierOutboxIntegrationTest.MinimalConfig.class)
public class SupplierOutboxIntegrationTest {
    ...
    public static class MinimalConfig {
        ...
        @Bean
        public SupplierEvaluationApprovedEventProcessor approvedEventProcessor(
            ...
            return new SupplierEvaluationApprovedEventProcessor(
                    taskRepository,
    ...
    @Autowired private SupplierEvaluationApprovedEventProcessor approvedEventProcessor;
    ...
    void setUp() {
        ...
        ruleSet.setEvaluatedRole(CompanyRole.SUPPLIER);
        ...
        ruleSet.getRules().add(createRule(ruleSet, "qualityPerformanceScore", "0.22", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "costCompetitivenessScore", "0.15", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "deliveryPerformanceScore", "0.21", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "capacityFlexibilityScore", "0.14", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "serviceResponsivenessScore", "0.10", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
        ruleSet.getRules().add(createRule(ruleSet, "supplyRiskComplianceScore", "0.18", com.apms.domain.score.enums.ScoreDirection.BENEFIT));
    ...
    @Test
    void testSupplierApprovalE2E() {
        Map<String, CriterionSnapshot> criteria = new HashMap<>();
        criteria.put("qualityPerformanceScore", CriterionSnapshot.builder().rawScore(new BigDecimal("80.0")).build());
        criteria.put("costCompetitivenessScore", CriterionSnapshot.builder().rawScore(new BigDecimal("70.0")).build());
        criteria.put("deliveryPerformanceScore", CriterionSnapshot.builder().rawScore(new BigDecimal("60.0")).build());
        criteria.put("capacityFlexibilityScore", CriterionSnapshot.builder().rawScore(new BigDecimal("50.0")).build());
        criteria.put("serviceResponsivenessScore", CriterionSnapshot.builder().rawScore(new BigDecimal("90.0")).build());
        criteria.put("supplyRiskComplianceScore", CriterionSnapshot.builder().rawScore(new BigDecimal("75.0")).build());
        // overallScore = 80*0.22 + 70*0.15 + 60*0.21 + 50*0.14 + 90*0.10 + 75*0.18 = 17.6 + 10.5 + 12.6 + 7.0 + 9.0 + 13.5 = 70.20

        RoleEvaluationVersion version = new RoleEvaluationVersion();
        version.setId(versionId);
        version.setEvaluationId(evaluationId);
        version.setEvaluatedRole(CompanyRole.SUPPLIER);
        ...
        assertThat(snapshot.getOverallScore()).isEqualByComparingTo(new BigDecimal("70.20"));
        assertThat(snapshot.getEvaluatedRole()).isEqualTo(CompanyRole.SUPPLIER);
