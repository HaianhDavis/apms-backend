...
public class SupplierEvaluationRevisionRequestedEventProcessor implements RoleEvaluationOutboxEventProcessorStrategy {
    ...
    public boolean supports(RoleEvaluationOutboxEventType eventType) {
        return eventType == RoleEvaluationOutboxEventType.SUPPLIER_EVALUATION_REVISION_REQUESTED;
    }
    ...
    public void process(RoleEvaluationOutboxEvent event) {
        ...
        auditLogService.log(event.getPayload().getActorAccountId(), AuditAction.ROLE_EVALUATION_REVISION_REQUESTED,
                ...
                "Revision requested for SUPPLIER evaluation: " + event.getPayload().getManagerFeedback());
    }
}
