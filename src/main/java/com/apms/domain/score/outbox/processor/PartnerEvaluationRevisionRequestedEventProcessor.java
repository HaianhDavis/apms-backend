...
public class PartnerEvaluationRevisionRequestedEventProcessor implements RoleEvaluationOutboxEventProcessorStrategy {
    ...
    public boolean supports(RoleEvaluationOutboxEventType eventType) {
        return eventType == RoleEvaluationOutboxEventType.PARTNER_EVALUATION_REVISION_REQUESTED;
    }
    ...
    public void process(RoleEvaluationOutboxEvent event) {
        ...
        auditLogService.log(event.getPayload().getActorAccountId(), AuditAction.PARTNER_EVALUATION_REVISION_REQUESTED,
                "ROLE_EVALUATION_DRAFT", event.getEvaluationId(),
                "Revision requested: " + event.getPayload().getManagerFeedback());
    }
}
