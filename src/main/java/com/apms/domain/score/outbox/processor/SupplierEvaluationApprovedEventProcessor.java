...
public class SupplierEvaluationApprovedEventProcessor implements RoleEvaluationOutboxEventProcessorStrategy {
    ...
    public boolean supports(RoleEvaluationOutboxEventType eventType) {
        return eventType == RoleEvaluationOutboxEventType.SUPPLIER_EVALUATION_APPROVED;
    }
    ...
    public void process(RoleEvaluationOutboxEvent event) {
        ...
        auditLogService.log(event.getPayload().getActorAccountId(), AuditAction.ROLE_EVALUATION_APPROVED,
                ...
                "Approved SUPPLIER evaluation. versionId=" + version.getId() + ", snapshotId=" + snapshot.getScoreSnapshotId() + ", overallScore=" + result.getOverallScore());
    }
}
