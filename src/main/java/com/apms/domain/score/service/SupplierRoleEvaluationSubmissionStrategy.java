...
/**
 * Synchronous submission strategy for SUPPLIER role evaluations.
 */
@Component
@RequiredArgsConstructor
public class SupplierRoleEvaluationSubmissionStrategy implements RoleEvaluationSubmissionStrategy {
    ...
    public void submit(RoleEvaluationDraft draft, ProjectTask task, ProjectTaskSubmission existingSubmission, SubmitRoleEvaluationRequest request, Long accountId) {
        ...
        if (!readiness.isStaffMaySubmit() || readiness.getAggregateCompletenessStatus() == EvaluationCompletenessStatus.INCOMPLETE) {
            throw new IllegalStateException("Data is INCOMPLETE. 6 canonical SUPPLIER criteria are required with sufficient evidence.");
        }
        ...
        RoleEvaluationOutboxPayload payload = RoleEvaluationOutboxPayload.builder()
                .eventId(eventId)
                .eventType(RoleEvaluationOutboxEventType.SUPPLIER_EVALUATION_SUBMITTED.name())
        ...
        RoleEvaluationOutboxEvent event = RoleEvaluationOutboxEvent.builder()
                ...
                .projectId(task.getProject().getId())
                .taskId(task.getId())
                .eventType(RoleEvaluationOutboxEventType.SUPPLIER_EVALUATION_SUBMITTED)
